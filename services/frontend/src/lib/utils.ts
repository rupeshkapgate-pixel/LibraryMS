...
export {
  getDaysOverdue as daysOverdue,
  getDaysRemaining as daysRemaining,
} from "./dateUtils";
...
export interface LocalLendingActivity {
  id: string;
  type: LocalLendingActivityType;
  at: string;
  bookTitle?: string;
  memberName?: string;
}
...
export function getLocalLendingActivity(): LocalLendingActivity[] {
  ...
  try {
    const parsed = JSON.parse(
      window.localStorage.getItem(LENDING_ACTIVITY_KEY) || "[]",
    );
...
export function recordLocalLendingActivity(
  activity: Omit<LocalLendingActivity, "at"> & { at?: string },
) {
  if (typeof window === "undefined") return;
  const existing = getLocalLendingActivity();
  const next: LocalLendingActivity = {
    ...activity,
    at: activity.at || new Date().toISOString(),
  };
  window.localStorage.setItem(
    LENDING_ACTIVITY_KEY,
    JSON.stringify([next, ...existing].slice(0, 100)),
  );
}
